...
namespace aruco
{
    cv::Mat impl__aruco_getRTMatrix(const cv::Mat& _rvec, const cv::Mat& _tvec)
    {
        ...
        for (int i = 0; i < 3; i++)
            Matrix.at<float>(i, 3) = _tvec.ptr<float>(0)[i];
        return Matrix;
    }
                                         ...
                                         float minerrorRatio)
    {
        ...
        {  // if no previous data, use from scratch
            cv::Mat rv, tv;
            auto solutions =
                IPPE::solvePnP_(Marker::get3DPoints(_msize), m, _cam_params.CameraMatrix, _cam_params.Distorsion);
        ...
        else
        {
            ...
            cv::solvePnP(Marker::get3DPoints(_msize), m, _cam_params.CameraMatrix, _cam_params.Distorsion, _rvec, _tvec,
                         true);
#else  // solve Pnp does not work properly in OpenCV2
            __aruco_solve_pnp(Marker::get3DPoints(_msize), m, _cam_params.CameraMatrix, _cam_params.Distorsion, _rvec,
                              _tvec);
#endif
        }
    ...
    bool MarkerMapPoseTracker::estimatePose(const vector<Marker>& v_m)
    {
        ...
        {
            if (_rvec.empty())
            {  // requires ransac since past pose is unknown
                cv::Mat rv, tv;
