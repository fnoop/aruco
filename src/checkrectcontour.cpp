...
namespace aruco
{
    ...
    public:
        ...
        {
            return fabs(_a * p.x + _b * p.y + _c) * inv_den;
        }
        int sign(cv::Point p)
        {
    ...
    void CheckRectContour::calculate_emax_between_points(const vector<cv::Point>& contorno, unsigned int inicial,
                                                         unsigned int final, float& errorMaximo, unsigned int& posicion)
    {
        ...
        for (unsigned int i = begin; i != final;)
        {
            ...
            if (i >= contorno.size())
                i = 0;  // if exceed the limits, go back to 0
        }
    }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else  // Es un cuadrilatero
                        {
                            ...
                            if (l1.sign(contorno[posicionMax2]) == l1.sign(contorno[posicionMax4]))
                                return false;
                            Line l2(contorno[posicionMax2], contorno[posicionMax4]);
                            if (l2.sign(contorno[posicionMax1]) == l2.sign(contorno[posicionMax3]))
                                return false;
    ...
    vector<cv::Point> CheckRectContour::getConvexRect(vector<cv::Point>& points, float thres)
    {
        ...
        if (check_rect_contour(points, thres, pres))
            return pres;
        else
            return {};
    }
