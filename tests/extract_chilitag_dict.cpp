...
uint64_t process(string file)
{
    ...
    if (im.total() != 100)
        throw std::runtime_error("required mini-no border");
...
int main(int argc, char** argv)
{
    ...
    std::sort(files.begin(), files.end(),
              [&](const string& s1, const string& s2) { return getNumber(s1) < getNumber(s2); });
